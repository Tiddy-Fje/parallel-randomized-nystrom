...
%%%
	...
	% MISSING CHARACTERISATION OF PROPERTIES ?
...
The results (Figure~\ref{fig:slow_fast_exp_decay}, left) show that BSRHT outperforms Gaussian Sketching for all ranks when $l$ is small (e.g., $l = 15$ or $l = 20$). As $l$ increases, Gaussian Sketching catches up and slightly surpasses BSRHT at higher ranks ($k > 80$). This demonstrates that BSRHT is more stable and effective with lower embedding dimensions but gradually loses its advantage as $l$ grows.

\paragraph{Fast Exponential Decay:}
For fast decaying spectra (Figure~\ref{fig:slow_fast_exp_decay}, right), Gaussian Sketching dominates at all ranks and embedding dimensions. While BSRHT's relative error reduces at higher ranks, it struggles to match Gaussian Sketching’s superior accuracy. This behavior highlights BSRHT's limitations in scenarios with rapid spectral decay.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{exp_decay_stability_custom.png}
    \caption{}
    \label{fig:slow_fast_exp_decay}
\end{figure}
\paragraph{Slow Polynomial Decay:} 
For slow polynomial decay (Figure~\ref{fig:poly_decay}, left), Gaussian Sketching and BSRHT exhibit similar trends at higher embedding dimensions ($l = 100, 300$), where both achieve smooth and consistent error reductions. However, at smaller embedding dimensions ($l = 30$), BSRHT shows slightly higher relative errors at lower ranks compared to Gaussian Sketching. 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{poly_decay_stability_custom.png}
    \caption{}
    \label{fig:poly_decay}
\end{figure}
\paragraph{MNIST}
For small embedding dimensions ($l = 30$), Gaussian Sketching and BSRHT exhibit similar performance, with BSRHT slightly outperforming Gaussian Sketching at lower ranks.
 However, Gaussian Sketching gains a marginal edge at higher ranks. As embedding dimensions increase ($l = 100$ and $l = 300$),the two methods perform almost the same, see Figure~\ref{fig:mnsit_decay}.
 \begin{figure}[h]
    \centering
...
\end{figure}
	...
	\WHILE{$h < m$}
		\FOR{$i = 0$ to $m - 1$ with step $2h$}
			...
			\STATE $\quad A[i:i+h, :] \gets A[i:i+h, :] + A[i+h:i+2h, :]$
			\STATE $\quad A[i+h:i+2h, :] \gets A[i:i+h, :] - 2 \cdot A[i+h:i+2h, :]$
		\ENDFOR
		\STATE $h \gets 2h$
	\ENDWHILE
	\IF{$\text{copy}$}
